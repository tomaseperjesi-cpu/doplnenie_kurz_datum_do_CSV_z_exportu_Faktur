...
with col1:
    file1 = st.file_uploader("Nahraj hlavný súbor (CSV alebo XLSX)", type=["csv", "xlsx"])
with col2:
    file2 = st.file_uploader("Nahraj XML export z Pohody", type=["xml"])

if file1 and file2:
    # 1. Načítanie hlavného súboru (CSV alebo XLSX)
    ...
    with exp_col1:
        ...
        st.download_button(
            "Stiahnuť výsledné CSV",
            data=csv,
            file_name="faktury_opravene.csv",
            mime="text/csv"
        )
    ...
    with exp_col2:
        ...
        excel_buffer.seek(0)
        st.download_button(
            "Stiahnuť výsledné XLSX",
            data=excel_buffer,
            file_name="faktury_opravene.xlsx",
